...
struct CachedPublishSubscribeLibraryTest : public testing::Test
{
    ...
    virtual void SetUp() override
    {
        ...
        terminalA   = make_terminal(leafA, YOGI_TM_CACHEDPUBLISHSUBSCRIBE, "A");
        terminalB   = make_terminal(leafB, YOGI_TM_CACHEDPUBLISHSUBSCRIBE, "B");
    ...
    void publish_receive(const char* data, unsigned bufferSize)
    {
        ...
        int res = YOGI_CPS_AsyncReceiveMessage(terminalA, buffer,
            bufferSize, helpers::ReceivePublishedMessageHandler::fn, &rcvMsgFn);
        EXPECT_EQ(YOGI_OK, res);

        do {
            res = YOGI_CPS_Publish(terminalB, data, strlen(data) + 1);
...
TEST_F(CachedPublishSubscribeLibraryTest, Cache)
{
    ...
    int res = YOGI_CPS_GetCachedMessage(terminalA, buffer, sizeof(buffer),
        &bytesWritten);
    EXPECT_EQ(YOGI_OK, res);
    EXPECT_STREQ("Hello", buffer);
    ...
    // buffer too small
    buffer[0] = 0;
    res = YOGI_CPS_GetCachedMessage(terminalA, buffer, 1, &bytesWritten);
    EXPECT_EQ(YOGI_ERR_BUFFER_TOO_SMALL, res);
    EXPECT_EQ('H', buffer[0]);
    EXPECT_EQ(1, bytesWritten);

    // receive cached message
    res = YOGI_CPS_AsyncReceiveMessage(terminalA, buffer,
