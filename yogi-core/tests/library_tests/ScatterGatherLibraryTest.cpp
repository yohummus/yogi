...
struct ScatterGatherLibraryTest : public testing::Test
{
    ...
    virtual void SetUp() override
    {
        ...
        terminalA   = make_terminal(leafA, CHIRP_TM_SCATTERGATHER, "A");
        terminalB   = make_terminal(leafB, CHIRP_TM_SCATTERGATHER, "B");
        terminalC   = make_terminal(leafC, CHIRP_TM_SCATTERGATHER, "C");
    ...
    void wait_until_system_stable()
    {
        while (true) {
            ...
            int res = CHIRP_SG_AsyncScatterGather(terminalA, nullptr, 0, nullptr,
...
TEST_F(ScatterGatherLibraryTest, SuccessfulScatterGatherOperation)
{
    ...
    int res = CHIRP_SG_AsyncReceiveScatteredMessage(terminalC, gathBuffer,
        sizeof(gathBuffer), helpers::ReceiveScatteredMessageHandler::fn,
        &rcvScatteredMsgFn);
    ASSERT_EQ(CHIRP_OK, res);

    char scatBuffer[10] = {0};
    int id = CHIRP_SG_AsyncScatterGather(terminalA, "Hi", 3, scatBuffer,
        sizeof(scatBuffer),
    ...
    res = CHIRP_SG_RespondToScatteredMessage(terminalC,
...
TEST_F(ScatterGatherLibraryTest, IgnoreScatteredMessage)
{
    ...
    res = CHIRP_SG_IgnoreScatteredMessage(terminalC,
...
{
    int id = CHIRP_SG_AsyncScatterGather(terminalA, "Hi", 3, nullptr, 0,
        helpers::ReceiveGatheredMessageHandler::fn, &rcvGatheredMsgFn);
    ASSERT_GT(id, 0);
...
{
    int res = CHIRP_SG_AsyncReceiveScatteredMessage(terminalB, nullptr, 0,
    ...
    res = CHIRP_SG_CancelScatterGather(terminalA, id);
    ASSERT_EQ(CHIRP_OK, res);
...
TEST_F(ScatterGatherLibraryTest, CancelReceiveScatteredMessageOperation)
{
    ...
    res = CHIRP_SG_CancelReceiveScatteredMessage(terminalC);
    ASSERT_EQ(CHIRP_OK, res);
