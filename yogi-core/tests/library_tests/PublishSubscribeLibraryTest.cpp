...
struct PublishSubscribeLibraryTest : public testing::Test
{
    ...
    virtual void SetUp() override
    {
        ...
        terminalA   = make_terminal(leafA, CHIRP_TM_PUBLISHSUBSCRIBE, "A");
        terminalB   = make_terminal(leafB, CHIRP_TM_PUBLISHSUBSCRIBE, "B");
    ...
    void publish_receive(const char* data, unsigned bufferSize)
    {
        ...
        int res = CHIRP_PS_AsyncReceiveMessage(terminalA, buffer,
            bufferSize, helpers::ReceivePublishedMessageHandler::fn, &rcvMsgFn);
        EXPECT_EQ(CHIRP_OK, res);

        do {
            res = CHIRP_PS_Publish(terminalB, data, strlen(data) + 1);
