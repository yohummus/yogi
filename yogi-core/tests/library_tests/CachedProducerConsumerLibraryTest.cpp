...
struct CachedProducerConsumerLibraryTest : public testing::Test
{
    ...
    virtual void SetUp() override
    {
        ...
        terminalA   = make_terminal(leafA, CHIRP_TM_CACHEDCONSUMER, "T");
        terminalB   = make_terminal(leafB, CHIRP_TM_CACHEDPRODUCER, "T");
    ...
    void publish_receive(const char* data, unsigned bufferSize)
    {
        ...
        int res = CHIRP_CPC_AsyncReceiveMessage(terminalA, buffer,
            bufferSize, helpers::ReceivePublishedMessageHandler::fn, &rcvMsgFn);
        EXPECT_EQ(CHIRP_OK, res);

        do {
            res = CHIRP_CPC_Publish(terminalB, data, strlen(data) + 1);
...
TEST_F(CachedProducerConsumerLibraryTest, Cache)
{
    ...
    int res = CHIRP_CPC_GetCachedMessage(terminalA, buffer, sizeof(buffer),
        &bytesWritten);
    EXPECT_EQ(CHIRP_OK, res);
    EXPECT_STREQ("Hello", buffer);
    ...
    // buffer too small
    buffer[0] = 0;
    res = CHIRP_CPC_GetCachedMessage(terminalA, buffer, 1, &bytesWritten);
    EXPECT_EQ(CHIRP_ERR_BUFFER_TOO_SMALL, res);
    EXPECT_EQ('H', buffer[0]);
    EXPECT_EQ(1, bytesWritten);

    // receive cached message
    res = CHIRP_CPC_AsyncReceiveMessage(terminalA, buffer,
