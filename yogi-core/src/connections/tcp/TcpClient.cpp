    ...
    boost::asio::ip::tcp::resolver::iterator epIt)
{
    ...
    if (!ec) {
        ...
        if (m_canceled) {
            m_connectOp.fire<CHIRP_ERR_CANCELED>(tcp_connection_ptr{});
        }
        else {
    ...
    // error
        ...
        BOOST_LOG_TRIVIAL(error) << "Async resolve operation failed: "
            << ec.message();
        m_connectOp.fire<CHIRP_ERR_RESOLVE_FAILED>(tcp_connection_ptr{});
    }
}
    ...
    boost::asio::ip::tcp::resolver::iterator epIt)
{
    ...
    // error
        ...
        if (ec == boost::asio::error::connection_refused) {
            m_connectOp.fire<CHIRP_ERR_CONNECTION_REFUSED>(
                tcp_connection_ptr{});
        }
        else if (ec == boost::asio::error::host_unreachable) {
            m_connectOp.fire<CHIRP_ERR_HOST_UNREACHABLE>(tcp_connection_ptr{});
        }
        else if (ec == boost::asio::error::network_down) {
            m_connectOp.fire<CHIRP_ERR_NETWORK_DOWN>(tcp_connection_ptr{});
        }
        else {
            m_connectOp.fire<CHIRP_ERR_CONNECT_FAILED>(tcp_connection_ptr{});
        }
    }
}
