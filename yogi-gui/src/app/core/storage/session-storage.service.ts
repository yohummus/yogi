...
export class SessionStorageService {
  private _store: yogi.SessionStorage;
  ...
  constructor(private _yogiService: YogiService, private _errorDisplayService: ErrorDisplayService) {
    ...
    .then(() => {
      this._store = new yogi.SessionStorage(this._yogiService.session);
    });
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      .catch((err) => {
        this._errorDisplayService.show('Storing data failed',
          `Could not store ${arr.byteLength} bytes of data in the session variable '${variable}':`
          + `\n${err}`);
        reject(err);
      });
    });
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      .catch((err) => {
        this._errorDisplayService.show('Reading data failed',
          `Could not read session variable '${variable}':\n${err}`);
        reject(err);
      });
    });
  }
